...
contract("AssetProxyRegistry", function([admin, owner]) {
  ...
  describe("registerAssetProxy", function() {
    it("should register proxy", async function() {
      ...
      expect(await this.registry.assetProxyOf(ERC20_PROXY_ID)).to.be.equal(this.erc20Proxy.address);

      await this.registry.registerAssetProxy(ERC721_PROXY_ID, this.erc721Proxy.address, { from: owner });
      expect(await this.registry.assetProxyOf(ERC721_PROXY_ID)).to.be.equal(this.erc721Proxy.address);
    });

    it("should revert if proxyId is 0", async function() {
      await shouldFail.reverting(this.registry.registerAssetProxy("0x00", this.erc20Proxy.address, { from: owner }));
    });
